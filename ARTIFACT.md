...
| **V10** | **M20's evidence is a string literal, not a captured response** | `grep -n 'structural' probes/probe_aggregation.py` then `grep -rn 'aggregate(' probes/*.py` | the `structural` field at line 190 is a fixed string emitted unconditionally, and the only `aggregate(` call in the whole of `probes/` is the pymongo `$group` at `probe_aggregation.py:103`, **against MongoDB** — no probe ever issues `aggregate`, `$group` or `distinct` against HCD. §8.4 |
...
`probes/probe_aggregation.py:190` and emitted **unconditionally**, regardless of server behaviour,
