...
| **L9** ⭑ | The two regime comparisons that carry the dossier's strongest architectural claim — that the per-byte coefficient is a regime artefact — are marked **UNDETERMINABLE** in `data/derived/inference.json`: their records kept a median and nothing else, so neither separation of support nor any interval can be computed from them. The claim rests on two bare medians. | **BOUNDS** the claim to a statement about medians | `probes/disk_regime_rerun.py` is written to close it, with rules R1–R4 fixed before execution and every observation retained; it has **not been run** |
