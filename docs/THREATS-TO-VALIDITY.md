...
| **L10** ⭑⭑ | **The regime claim rests on an asymmetry.** The decomposing engine was measured in three regimes — memtable (M3), dataset on disk (M10), read forced onto SSTables (M15) — and the dossier concludes from their spread that the per-byte coefficient is a *regime artefact*. The comparator was measured in **one**: every MongoDB record in `data/raw/` is cache-resident. Whether WiredTiger shows the same sensitivity is unknown, and three outcomes are open, including one in which the mutation-axis direction inverts at some size. | **BOUNDS** the regime claim to one engine, and leaves the mutation axis unbounded outside the cache | `probes/mongo_regime_rerun.py` is written to close it, with rules R1–R4 fixed before execution, eviction verified per arm rather than assumed, and every observation retained; it has **not been run** |
| **L9** ⭑ | The two regime comparisons that carry the dossier's strongest architectural claim — that the per-byte coefficient is a regime artefact — are marked **UNDETERMINABLE** in `data/derived/inference.json`: their records kept a median and nothing else, so neither separation of support nor any interval can be computed from them. The claim rests on two bare medians. | **BOUNDS** the claim to a statement about medians | `probes/disk_regime_rerun.py` is written to close it, with rules R1–R4 fixed before execution and every observation retained; it has **not been run**. A second defect of the same record, found while writing that probe: `findings_disk_rf3.json` carries **no host fingerprint** — its host is known from campaign context and not from the file, so nothing in it forbids comparing it with a measurement taken elsewhere. The new probe records hostname, CPU, memory and load average at start and end |
