...
mutation cost tracks *unchanged* indexed content. **MongoDB won the majority of the measured axes** —
single-field mutation (44× to 79× on the stack, 40× to 72× engine-to-engine), point read by `_id`
(≈19.6×), filtered search on a field it was given an index for (23.7×), aggregation (602× against
HCD's Data API path), and exact counting, which HCD's Data API simply refuses above 1 000 documents.
HCD won two: filtered search on a field nobody declared an index on, against a MongoDB that was never
...
article's own §10 standard would disqualify somebody else's benchmark, and which disqualify these
...
1. **Step 4 of the five-operation model is contradicted.** The article's §3 numbered list and its
...
38. **The campaign declines to support the axes the article claims for HCD.** Its own "what this
    ...
    not close it.
39. **The article's own §10 standard is not met by this comparison.** The adversarial audit scores the
...
|---|---|---|
| **Concurrency** | Never measured. Every probe in every campaign is a **single sequential closed-loop client**. | This violates requirement 1 of the article's own §10 standard for a defensible benchmark. Contention, queueing and lock behaviour are entirely unknown on both engines; the audit scores the dossier at about 2.5 of §10's six requirements. |
...
2. **Never quote the 602× as "HCD is 602× slower".** The same engine aggregates in ~2 s in native CQL;
   the gap is an API surface, and reaching the CQL path costs the document model.
