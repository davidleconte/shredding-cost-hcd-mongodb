...
campaigns over two days produced 22 numbered measurements (M1–M8, M10–M23; no M9 was ever assigned) against the two stacks above, on
...
|---|---|---|---|
| **Single-field mutation, per indexed KiB** — M13, M14 | HCD 0.7775 ms/KiB via the Data API, 0.7037 CQL-direct — both Paxos-guarded — against MongoDB 0.0176 (wildcard) / 0.0098 (default): **44× / 79×** stack-to-stack, **40× / 72×** with the tier stripped — *not* engine-to-engine, the CQL-direct arm keeps the Paxos-guarded conditional (threat C1-bis). **Memtable-resident rate only** | **Direction certain** — 19 of 20 Data-API cells and 18 of 20 CQL cells disjoint; the three exceptions are all 8 KiB pass 2, caused by a MongoDB tail | ⛔ **Voids the precision, not the direction:** the slopes are fitted on **pass 1 only**, undeclared until now ([ARTIFACT.md §8.5](ARTIFACT.md)), and MongoDB's denominator is near-flat (r² 0.78 / 0.88 against HCD's 0.997) — *"the weakest quantity in the comparison"* ([METHODOLOGY.md §6](METHODOLOGY.md)). ⚠︎ The coefficient behind it collapses ×5.94 → ×1.52 on SSTables (M15). Durability matching favoured HCD; HCD lost anyway |
...
| [`data/raw/`](data/raw/) | The 39 JSON files the probes wrote, unedited, including the files a later run refuted; `data/MANIFEST.sha256` carries one SHA-256 per file and CI re-verifies it |
...
| [`docs/article/`](docs/article/) | The article under verification, in its current two-volet form: [Part I](docs/article/article-part1-what-a-mutation-costs.html) (what a mutation costs) and [Part II](docs/article/article-part2-when-the-index-is-current.html) (when the index is current). They carry the corrections this repository forced, including the three retractions of 18 September — D12's annulment with its R3 reserve, D9's refutation, and D7's decomposition to 13.398 ms per page — applied in seven declared gestures ordered by `.github/WORK-ORDER-html.fr.md` and verified against it. Where a superseded reading was load-bearing it is quoted and then refuted in place rather than deleted. The single-file `article-corrected.html` they replace is removed |
