...
against a MongoDB that was never given one — 21.7× **at the median, and this is the one comparison in the
dossier whose two samples overlap**, so what HCD demonstrably buys here is *predictability*, not uniform
speed (see the note on that row below) — and search-index freshness, where its index sits on
the write path.
...
| **Filtered search, undeclared field** (1 M documents) — [M16](RESULTS.md#3-measurements-m1m23) | p50 376.698 ms — collection scan, no index on the queried field | p50 17.376 ms — automatic SAI, nothing declared | HCD, at the median only | 21.7× (medians); honest interval **[0.46×, 65.8×]**, which **contains 1.0** | **The only comparison in this dossier where the two samples overlap, and it is the one axis HCD wins.** HCD spans [13.113, 28.697] ms over n = 50; MongoDB-default spans [13.335, **863.381**] ms over n = 50 — a collection scan whose cost depends on where in the collection the match happens to sit. MongoDB-default's *fastest* observation (13.335 ms) is faster than HCD's *median* (17.376 ms), so separation of support does **not** license the 21.7×, and no test of it is possible because the raw series were not kept ([docs/STATISTICS.md](docs/STATISTICS.md)). The defensible claim is **predictability**: HCD answers in ~17 ms whatever the document, MongoDB-default anywhere from 13 ms to 863 ms. That is a different claim from "22× faster", and a weaker one. It is also a win over a MongoDB that was **never given the index** (challenge C12) — the dispensation from index design is an operational property, not engine speed; the same MongoDB with a wildcard index answers in 0.732 ms with **disjoint** support (next row). [findings_rs_hcd.json](data/raw/findings_rs_hcd.json), [findings_rs_mongo.json](data/raw/findings_rs_mongo.json) |
