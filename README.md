...
| **Aggregation** — M23 | MongoDB `$group` p50 222.711 ms against the Data API's client scan-and-aggregate 133 984.229 ms: **602×**. HCD's own native CQL sweep: 2 011.399 ms — **66.6× between HCD's two paths**, 9.0× against MongoDB | **Direction certain** — [455.6×, 625.6×] — but on **n = 3**; its p95/p99 are not quotable | ⛔ **A capability gap, not a latency race**: never quote "HCD is 602× slower". The same engine, host and corpus aggregate in ~2 s through CQL; the 134 s belongs to a tier with no aggregation surface. And the CQL arm means abandoning the document model and pre-designing the partition key. ⚠︎ Unequal n (15 vs 3) |
| **Exact count** — M21, M22 | MongoDB answers (115.009 ms exact, 12.659 ms indexed); HCD **cannot** above 1 000 documents, and its estimate returned **0** against a true 200 000 | **Structural, not statistical** — HCD produced no observations because the operation is refused | ⚠︎ MongoDB wins by *answering*, not cheaply: its exact count is a scan. The zero is also direct proof this campaign ran in a **cache regime** |
