...
**MongoDB won the majority of the measured axes**: single-field mutation (44× to 79× stack-to-stack,
40× to 72× engine-to-engine), point read by `_id` (≈19.6×), filtered search on a field it was given an
index for (23.7×), aggregation (602× against HCD's Data API path) and exact counting, which HCD's Data API
refuses above 1 000 documents. HCD won two: filtered search on a field nobody declared an index on,
...
| **Measurements** | Numbered **M1–M23** across 7 campaigns. The register holds M1–M8 and M10–M23: no M9 was ever assigned, and it is listed as missing rather than silently skipped |
...
| [`docs/campaigns/`](docs/campaigns/) | The seven campaign reports, 01–07, in French, as written at the time. |
| [`docs/adr/ADR-001-modelling-policy.md`](docs/adr/ADR-001-modelling-policy.md) | The modelling policy and the evidence register the Mxx identifiers come from. The sole evidence for M6 and M8 lives here rather than in a raw file — the weakest traceability in the register. |
